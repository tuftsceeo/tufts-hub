...
def log_request(message, style="info"):
    ...
    log_container._dom_element.scrollTop = log_container._dom_element.scrollHeight
...
def display_pokemon(data):
    ...
    for t in types:
        ...
        type_badges.append(
            web.span(
                f"{type_emoji.get(type_name, '❓')} {type_name_cap}",
                className=f"type-badge type-{type_name}"
            )
        )
    ...
    card_content = web.div(
        ...
        web.div(
            ...
            web.audio(
                ...
                controls=True
            ) if cry_latest else web.p("(No cry available)", className="hint"),
...
async def handle_search(event):
    ...
    try:
        ...
        response = await fetch(
            f"/proxy/pokeapi/pokemon/{pokemon_name}",
            method="GET"
        )
        
        if response.status == 404:
            log_request(f"❌ 404 Not Found - '{pokemon_name}' doesn't exist!", "error")
            ...
            if isinstance(data, str):
                ...
                card.innerHTML = "<p class='error-message'>Error parsing response</p>"
                return
            ...
            if isinstance(pokemon_found, str) and len(pokemon_found) > 0:
                # Capitalize first letter (MicroPython compatible).
                pokemon_found = pokemon_found[0].upper() + pokemon_found[1:].lower()
            
            log_request(f"✅ 200 OK - Found {pokemon_found}!", "success")
            display_pokemon(data)
        else:
            log_request(f"⚠️ {response.status} - Unexpected response", "warning")
